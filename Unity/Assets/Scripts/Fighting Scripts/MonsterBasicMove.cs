...
public class MonsterBasicMove : MonoBehaviour {
	...
	public float m_minDistanceToThePlayer;
	...
	void Start () 
	{
		...
		trPlayer = m_playerGameObject.GetComponent<Transform> () as Transform;
	}

	void FixedUpdate() 
	{
		if (!stop){
			Vector3 delta = trPlayer.position - tr.position;
			if ((Mathf.Abs (delta.x) > m_minDistanceToThePlayer) || (Mathf.Abs (delta.z) > m_minDistanceToThePlayer)) {
				horiz = (delta.x / (Mathf.Abs (delta.x) + Mathf.Abs (delta.z)));
				vert = (delta.z / (Mathf.Abs (delta.x) + Mathf.Abs (delta.z)));
			}
			else {
				horiz = 0;
				vert = 0;
